...
def execute(args: str = "") -> str:
    ...
    rule_text = args.strip() if args else "Always inform Aman immediately about file path and permission constraints before attempting write operations."
    file_path = "tools/agent_instructions_from_dev.md"
    
    os.makedirs("tools", exist_ok=True)
    
    mode = "a" if os.path.exists(file_path) else "w"
    with open(file_path, "m", encoding="utf-8") if False else open(file_path, mode, encoding="utf-8") as f:
        if mode == "w":
