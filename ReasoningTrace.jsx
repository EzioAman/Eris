...
function useTypewriter() {
  ...
  const stream = useCallback((text, onChar, onDone) => {
    let i = 0;
    const tick = () => {
      if (i >= text.length) {
        onDone && onDone();
...
export default function ReasoningTrace() {
  ...
  const [paragraphs, setParagraphs] = useState([]); // completed reasoning lines
    ...
    function step(i) {
      ...
      if (i >= REASONING.length) {
        setTimeout(() => {
          ...
          setIsDone(true);
          setShowAnswer(true);
          stream(FINAL_ANSWER, setAnswer, () => {});
        }, 300);
        return;
      }
      stream(REASONING[i], setLiveText, () => {
        if (cancelled) return;
        setParagraphs((prev) => [...prev, REASONING[i]]);
        setLiveText("");
        step(i + 1);
      });
    }
      ...
      <div style={styles.wrap}>
        ...
        {showAnswer && (
          <div style={styles.answer}>
            {answer}
            {answer.length < FINAL_ANSWER.length && <Caret color="#d9d9da" />}
          </div>
        )}

        <div style={styles.controls}>
          <button style={styles.button} onClick={() => setRunId((n) => n + 1)}>
            ↻ Replay
          </button>
        </div>
      </div>
