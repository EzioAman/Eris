      ...
      <div className="flex-1 min-w-0 space-y-2">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            className={cn(
              'cursor-pointer text-[11px] font-mono font-medium px-2 py-0.5 rounded-full border transition-colors flex items-center gap-1.5 hover:border-emerald-500/40',
              isDarkMode
                ? 'border-white/10 bg-white/5 text-neutral-300 hover:bg-white/10'
                : 'border-slate-200 bg-slate-100 text-slate-700 hover:bg-slate-200'
            )}
            title={`Generated by ${formatModelName(msg.activeModel || activeModel)} (Click to switch model)`}
          >
            <span className="size-1.5 rounded-full bg-emerald-500 shrink-0" />
            <span>
              {formatModelName(msg.activeModel || activeModel || 'OpenRouter Auto')}
            </span>
          </button>
