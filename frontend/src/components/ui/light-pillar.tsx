...
export const LightPillar: React.FC<LightPillarProps> = ({
  ...
  useEffect(() => {
    ...
    const isLowEndDevice = isMobile || (navigator.hardwareConcurrency ? navigator.hardwareConcurrency <= 4 : false);
  ...
  }, [webGLSupported, quality, interactive, intensity, glowAmount, pillarWidth, pillarHeight, noiseIntensity, lightMode, pillarRotation, topColor, bottomColor]);
  ...
  if (!webGLSupported) {
    return (
      <div
        className={`w-full h-full absolute top-0 left-0 flex items-center justify-center bg-gradient-to-b from-purple-950/20 via-black to-slate-950 text-neutral-500 text-xs ${className}`}
        style={{ mixBlendMode }}
      >
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(82,39,255,0.15),transparent_70%)]" />
      </div>
    );
  }
