...
export default function Strands({
  ...
  useEffect(() => {
    ...
    const initialColors = propsRef.current.colors || colors;
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uColors: { value: buildPalette(initialColors) },
        uColorCount: { value: Math.min(initialColors.length, MAX_COLORS) },
        uStrandCount: { value: Math.min(propsRef.current.count ?? count, MAX_STRANDS) },
        uSpeed: { value: speed },
    ...
    const glassProgram = new Program(gl, {
      ...
      uniforms: {
        ...
        uRadius: { value: 0.46 * (propsRef.current.glassSize ?? glassSize) },
    ...
    function resize() {
      ...
      const resW = gl.drawingBufferWidth || w;
      const resH = gl.drawingBufferHeight || h;
      program.uniforms.uResolution.value = [resW, resH];
      renderTarget.setSize(resW, resH);
      glassProgram.uniforms.uResolution.value = [resW, resH];
    }
